...
export class ScheduledRouteController {
  ...
  @ApiOperation({
    summary: 'Obtener vehículos disponibles sin rutas asignadas',
    description:
      'Obtiene vehículos activos con conductores asignados que no tienen rutas programadas',
  })
  @ApiResponse({
    status: 200,
  ...
  async getAvailableVehicles() {
    try {
      return await this.scheduledRouteService.getAvailableVehiclesWithoutRoutes();
    } catch (error) {
      if (error instanceof BadRequestException) {
        throw error;
      }
